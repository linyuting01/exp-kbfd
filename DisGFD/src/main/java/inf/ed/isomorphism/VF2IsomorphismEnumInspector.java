...
public class VF2IsomorphismEnumInspector<V1 extends Vertex, E1 extends Edge, V2 extends Vertex, E2 extends Edge> {

	static Logger log = LogManager.getLogger(VF2IsomorphismEnumInspector.class);
	...
	ConcurrentLinkedQueue<Int2IntMap> matches;
	...
	public VF2IsomorphismEnumInspector() {
	}
	...
	public VF2IsomorphismEnumInspector(Graph<V1, E1> query, int du, Graph<V2, E2> graph,
		...
		this.matches = new ConcurrentLinkedQueue<Int2IntMap>();
		this.cands = new ConcurrentLinkedQueue<Integer>();
		this.cands.addAll(candidates);
		log.info("isomorphism test begin:" + this.cands.size() + ", du = " + du);
	}
	...
	private class WorkerThread extends Thread {
		...
		public void run() {
			...
			try {
				while (!cands.isEmpty()) {
					if (cands.size() % 100 == 0) {
						log.info("current cands size = " + cands.size());
					}
					int can = cands.poll();
					isSubgraphIsomorphic(g1, du, g2, can);
				}
